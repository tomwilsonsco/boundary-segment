...
def create_6panel_plot(basename, chip_path, mask_path, pred_path, parcels_gdf, 
                       pred_lines_gdf, output_path):
    ...
    if pred_lines_gdf is not None and len(pred_lines_gdf) > 0:
        pred_lines_filtered = filter_geometries_to_bounds(pred_lines_gdf.copy(), bounds_geom, crs)
    else:
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Generate UNet example screenshots")
